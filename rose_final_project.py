...
# Summarize team statistics by grouping data at the team level and summing 'Goals' (Gls) and 'Expected Goals' (xG)
team_stats = combined_df.groupby(level=0)[['Gls', 'xG']].sum()

# Create a scatter plot of 'Expected Goals' (xG) vs. 'Goals Scored' (Gls)
...
# Calculate the mean goals scored by each team and sort the values
mean_goals = combined_df.groupby(level=0)['Gls'].mean().sort_values()

# Create a horizontal bar plot of mean goals scored by team
plt.figure(figsize=(8, 6))
...
# Merging DataFrames with suffixes to handle overlapping columns
...
# Check if 'Formation' is in full_df by printing the column names after merging
print("Columns in full_df after merging:")
print(full_df.columns)

# Save the merged DataFrame to an Excel file for manual inspection
full_df.to_excel('/content/drive/My Drive/Project/full_df_output.xlsx', index=True)
print("Full DataFrame saved for inspection.")

# Extract and store data for 'Arsenal' from the merged DataFrame
arsenal_combined = full_df.loc['Arsenal']

# Feature Engineering

# Separate features by dropping the target column 'Formation'
...
grid_search = GridSearchCV(svm_model, param_grid, cv=5, scoring=scoring, refit='accuracy', return_train_score=True)
...
# Accessing cross-validation scores from the grid search results
svm_cv_scores = grid_search.cv_results_['mean_test_accuracy']

# Print other metrics using grid_search.cv_results_
print(f"SVM 5-fold Cross-Validation Accuracy: {svm_cv_scores.mean()}")
print(f"Precision: {grid_search.cv_results_['mean_test_precision_weighted'][grid_search.best_index_]}")
print(f"Recall: {grid_search.cv_results_['mean_test_recall_weighted'][grid_search.best_index_]}")
print(f"F1-Score: {grid_search.cv_results_['mean_test_f1_weighted'][grid_search.best_index_]}")
...
arsenal_combined = full_df.loc['Arsenal'].copy()
...
# Label Encoding for target if necessary
# If formations are categorical, encode them
target_le = LabelEncoder()
target = target_le.fit_transform(target)
